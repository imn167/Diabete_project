...
proc freq data=malib.missing; /* voir proc mi qui donne un meilleur résumé peut-être*/
run; 
title;

/* On supprimeles variables indiquant la seconde mesure de pression arterielle. 
Les individus comportant des données manquantes après suppression des 2 variables sont aussi supprimés car
ça ne concerne que 28 individus en tout*/
...
/*tentative d'un boxplot pour toutes les variables (même methode que dans R) */
...
title "Resumé statistique des variables quantitatives de la table de données de 375 individus et 21 variables";
proc means data=malib.Diabete_clear(keep= _numeric_) maxdec= 2 min q1 median mean q3 max std;
run;
title;
...
proc sgpanel data=malib.diabete_fin;
panelby gender / layout=columnlattice onepanel colheaderpos=bottom novarname;
histogram ratio_wh ;
run;
title;
...
Le diabète type 2 semble être lié à l'age 
*/
...
/* -------------------	ANALYSE DE VARIANCE et Ttest -------------------*/

/* Test de comparaison de moyenne  

-Verification des hypothèses de normalité en traçant la distribution de la gaussienne associée à la variable
	quantitative 
- Si normalité, verification du plan equilibré 
*/


/*----------- ANALYSE DE L'INFLUENCE DU GENRE SUR L'HEMOGLOBINE ------------- */ 


proc means data= malib.diabete_fin mean std; /* Plan equilibré */
class gender ;
var glyhb;
run;

proc univariate normal data=malib.diabete_fin;
var glyhb; class gender ; run;

proc sgpanel data=malib.diabete_fin;
panelby gender / layout=columnlattice onepanel colheaderpos=bottom novarname;
histogram glyhb ;
density glyhb;						/*	 Pas de distribution normale donc impossible d'effectuer le Ttest
								On effectue un test de Man-Withney
								*/
run;
...
proc npar1way data=malib.diabete_fin wilcoxon;
    class gender;
    var glyhb;
run;

/* Pas de différence significatives entre hommes et femmes */

/* ----------- Analyse hémoglobine et age -------------- */
proc sgplot data=malib.Diabete_Fin;
histogram glyhb;
density glyhb;
run;
...
proc univariate normal data=malib.diabete_fin;
var glyhb ; run;
/* Pas de normalité des sous échantillon donc test de Test de Kruskal et Wallis*/

title "Distribution de l'hémoglobine glyquée en fonction des tranches d'age et test de normalité";
proc sgpanel data=malib.diabete_fin;
panelby age_code / columns= 3;
histogram glyhb ;
density glyhb;
run;
title;
...
title "Visualisation d'une présence ou non d'une distribution normale pour la variable d'hémoglobine glyquée";
proc sgplot data=malib.Diabete_Fin;
histogram imc;
density imc;
run;
...
proc npar1way  data=malib.Diabete_Fin;
class age_code; var imc; run;


/* -------- Analyse de variance pour l'imc en fonction de la tension arterielle ------- */
Proc sgpanel data=malib.Diabete_Fin nocycleattrs;
panelby bp_code / layout=columnlattice onepanel colheaderpos=bottom ; 
vbox imc ; run;
...
proc npar1way  data=malib.Diabete_Fin;
class bp_code; var imc; run;

/* Seule l'influence de la pression est visible */ 
Proc sgpanel data=malib.Diabete_Fin nocycleattrs;
panelby age_code / layout=columnlattice onepanel colheaderpos=bottom novarname; 
...
var glyhb hdl bp_1s bp_1d ratio_wh IMC chol;
run;
...
proc sgplot data=malib.diabete_fin; 
scatter X = bp_1d Y = glyhb ;
run;

proc sgplot data=malib.diabete_fin; 
scatter X = ratio_wh Y = chol ;
run;
...
proc freq data=malib.diabete_fin;
tables IMC_code * gender / norow nocol nopercent;
run;
...
proc freq data=malib.diabete_fin;
tables bp_code * gender / norow nocol nopercent;
run;

/*Tension arterielle de stade 2 pour plus de femmes*/
...
/* deux modèles différents + estimation des proba pour les interprétations */
...
format Bp_code;
   ...
   estimate "Pr prob Bp_code = 4 at hdl=50" intercept 1 age 46 IMC 28.83 / ilink category='4';
   estimate "Pr prob Bp_code = 4 at hdl=70" intercept 1 age 46 IMC 35 / ilink category='4';
   estimate "Pr prob Bp_code >=3 at hdl=50" intercept 1 age 46 IMC 28.83 / ilink category='3';
   estimate "Pr prob Bp_code >=3 at hdl=70" intercept 1 age 46 IMC 35 / ilink category='3';
   estimate "Pr prob Bp_code >=2 at hdl=50" intercept 1 age 46 IMC 28.83 / ilink category='2';
   estimate "Pr prob Bp_code >=2 at hdl=70" intercept 1 age 46 IMC 35 / ilink category='2';
